...
double cumError;
double rateError;
double tempSetPoint = 33.0;  // the target temperature for the enclosure which will buffer the OCXO oven.
...
void printHeader2_ToSerial()
{
  ...
  Serial.print(F("Rev. 3.7.0 paulv mods")); // paulv: added my mod indication
...
void control_fan() {
  ...
  int pid = (Kp * tempError) + (Ki * cumError) + (Kd * rateError);
  // use the pwm_baseline as the minimum pwm to keep the fan running
