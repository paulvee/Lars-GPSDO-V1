/*
  ...
  * added a PID fan controller that uses the enclosure temp sensor as input to drive a fan
  * 
...
// Fan controller
const int fan_pin = 6;    // Fan PWM output.  Pin 9 should also work but does NOT!
double ambient_t;         // ambient temperature
double pid;               // The result of the PID controller
int duty_cycle;           // PWM value for the fan, also pid
double tempError;
double lastError;
double cumError;
double rateError;
double tempSetPoint = 42.5;  // (v) the target temperature for the enclosure which will buffer the OCXO oven.
int pwm_baseline = 25;    // 20 is minimum, 25 is safer. Low PWM to keep the fan running and baseline
int max_pwm = 255;        // maximum PWM value
// PID gain factors. These work for me but could probably use a bit of further tuning depending on your hardware
// Due to the large intertia of the system, the Kp gain must be high, but the damping (Ki) can be very low.
// There are also only very minor jumps or transistions so Kd can be low as well.
double Kp = 150.0; // x 150
double Ki = 0.5;   // y
double Kd = 2.0;   // x


int warmUpTime = 60; // gives 1 minute hold during eg OCXO or Rb warmup. Set to eg 3 for VCTCXO
 	...
 	break;
      ...
      case F:      
        ...
        switch (zz) {
          ...
          Serial.print(F("Cool Baseline ")); Serial.print("\t"); Serial.println(tempSetPoint);
...
{
Serial.print(F("\n\rArduino GPSDO with 1ns TIC by Lars Walenius")); // paulv: added a new line in front
}
...
void printHeader2_ToSerial()
{
  ...
  Serial.print(F("Rev. 3.7.1 paulv mods")); // paulv: added my mod indication
...
{ 
  Serial.print("\t");    // *** paulv
  Serial.print("duty_cycle");   // *** paulv added fan pwm duty cycle to report
  Serial.print("\t");
  Serial.print(F("time"));
  Serial.print("\t");               
  Serial.print(F("ns"));
...
{  
  Serial.begin(9600);
  Serial.println("GPSDO starting up");
  ...
  // change the standard PWM frequency of 976.56 Hz for pins 5 and 6, we use 6 
  TCCR0B = TCCR0B & B11111000 | B00000010; // set timer 0 divisor to 8 for a PWM frequency of 7,812.50 Hz
  // The Noctua fan I'm using has a target frequency of 25KHz, with an acceptable range between
  // 21KHz and 28KHz, but that's not possible.
  Serial.println("kick starting the fan");
  duty_cycle = 100;
  analogWrite(fan_pin, duty_cycle); // kick-start the fan
  delay(5000);
...
void control_fan() {
  ...
  ambient_t = temperature_to_C(tempADC2_IIR,temperature_Sensor_Type/10); // use the GPSDO sensor

  // run the PID calculation
  // no need to bother calculating the elapsed time, it is always 1 s
  tempError = ambient_t - tempSetPoint; // determine error away from setpoint, make it a positive delta
  // this is analog to using the "reversed" mode
  cumError += tempError; // compute the integral
  rateError = (tempError - lastError); // compute the derivative

  pid = (Kp * tempError) + (Ki * cumError) + (Kd * rateError); // calculate pid = duty_cycle
  duty_cycle = pid; // pid is double, DC is int
  // set the min-max boundaries
  if (duty_cycle < pwm_baseline) { duty_cycle = pwm_baseline; } // lowest speed, also avoid negative numbers
