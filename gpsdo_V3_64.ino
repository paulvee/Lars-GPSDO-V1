...
// gain factors. These work very well for me but could probably use a bit of further tuning depending on your hardware
float Kp = 6;
float Ki = 0.2;
float Kd = 1;
...
void printHeader2_ToSerial()
{
  ...
  Serial.print(F("Rev. 3.6.4 paulv mods")); // paulv: added my mod indication
...
void control_fan() {
  ...
  int pid = (Kp * temp_error) + (Ki * integral) + (Kd * derivative);
...
void loop() 
{
  ...
  {
    calculation();
    // to reduce the run-up of the PID controller during the warm-up period,
    // wait with activating the fan controller until this period passed
    if (time > warmUpTime){
      control_fan(); // *** paulv
    }
    getCommand();
