...
int warmUpTime = 120; // ****was 300 gives five minutes hold during eg OCXO or Rb warmup. Set to eg 3 for VCTCXO
...
// **** modified to add a digital decharge of C1
ISR (TIMER1_CAPT_vect)
{
  timer1CounterValue = ICR1;  // read the captured timer1 200ns counter value
//  digitalWrite(probe, 1);     // create a signal to probe the scope from
  TIC_Value = analogRead(A0); // ns value
//  digitalWrite(probe, 0);     // stop the probe
  pinMode(decharge,OUTPUT);   // change from hi-Z to output
  digitalWrite(decharge, 0);  // start de-charging of C1
  PPS_ReadFlag = true;
  delayMicroseconds(10);      // allow sufficient time to fully drain the capacitor
  pinMode(decharge,INPUT);    // revert back to hi-Z
}
...
// ***** fix the swapping of the two sensors
...
//  Serial.print("\t");   
  ...
  { 
   if (PPSlocked == 0) {Serial.print(F("****"));} // changed from NoLock
...
{
  Serial.print(F("Rev. 3.1 paulv mods 170801"));
